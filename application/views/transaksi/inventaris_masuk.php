<label class="" style="width:100%">
    <form action="javascript:ch()" method="post">
        ...
        <div class="form-group row" style="padding-top: 0px">
            <label for="colFormLabel" class="col-sm-2 col-form-label">Posisi</label>
            <div class="col-sm-3">
                <input type="text" class="form-control" id="posisi" required>
            </div>
        </div>
        <div class="form-group row" style="padding-top: 0px">
            <label for="colFormLabel" class="col-sm-2 col-form-label">Penanggung Jawab</label>
            <div class="col-sm-3">
                <input type="text" class="form-control" id="p_jawab" required>
            </div>
...
<script type="text/javascript">
    ...
    function tambah() {
        ...
        Newcell3.innerHTML = h_beli;
        Newcell4.innerHTML = keterangan;
        Newcell5.innerHTML = posisi;
        Newcell6.innerHTML = p_jawab;
    ...
    function clearform() {
        ...
        document.getElementById("p_jawab").value = "";
    }
    ...
    function loadview() {
        location.href = "<?= base_url('transaksi/t_inventaris_masuk_c/detail_t') ?>";
    }
    ...
    function sub() {
        $.ajax({
            type: 'POST',
            url: '<?= base_url('transaksi/t_inventaris_masuk_c/tambahdetail') ?>',
            data: {
                Id_Detail: dt_id,
            },
            dataType: 'json'
    ...
    function getiddetail() {
        ...
        $.ajax({
            type: 'POST',
            url: '<?= base_url('transaksi/t_inventaris_masuk_c/getiddetail/') ?>',
            async: false,
            dataType: 'json',
            success: function(data) {
                id = data[0].Id_Detail;
    ...
    function myfun() {
        ...
        for (var i = 1; i < totalRows - 1; i++) {
            $.ajax({
                type: 'POST',
                url: '<?= base_url('transaksi/t_inventaris_masuk_c/tambahdata/') ?>',
                async: true,
                data: {
                    k_barang: table.rows[i].cells[0].innerHTML,
                    h_beli: table.rows[i].cells[2].innerHTML,
                    keterangan: table.rows[i].cells[3].innerHTML,
                    posisi: table.rows[i].cells[4].innerHTML,
                    p_jawab: table.rows[i].cells[5].innerHTML,
                    id_d: dt_id
                },
                dataType: 'json',
                success: function(data) {}
